...
export async function listEstados(): Promise<Estado[]> {
  ...
  if (!response.ok) {
    throw new Error(notConfiguredMessage("/estados"));
  }
...
export async function listMunicipios(estadoId: string): Promise<Municipio[]> {
  ...
  const response = await fetch(
    `${API_BASE_URL}/municipios?estado_id=${estadoId}`,
  );
  if (!response.ok) {
    throw new Error(notConfiguredMessage("/municipios"));
  }
...
export async function listBairros(municipioId: string): Promise<Bairro[]> {
  ...
  const response = await fetch(
    `${API_BASE_URL}/bairros?municipio_id=${municipioId}`,
  );
  if (!response.ok) {
    throw new Error(notConfiguredMessage("/bairros"));
  }
...
export async function listEscolasByBairro(bairroId: string): Promise<Escola[]> {
  ...
  if (!response.ok) {
    throw new Error(notConfiguredMessage("/escolas"));
  }
