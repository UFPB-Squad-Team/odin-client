...
import { listCamadas } from "@/services/geospatial-api";
import { buildMockCollection } from "@/services/geospatial-mock-data";
import type {
...
function normalizeEstadoIdToUf(
  ...
  if (trimmed.length === 2 && UF_TO_IBGE_STATE_CODE[trimmed]) {
    return trimmed;
  }

  if (IBGE_STATE_CODE_TO_UF[trimmed]) {
    return IBGE_STATE_CODE_TO_UF[trimmed];
  }
...
async function fetchIbgeMunicipalities(
  ...
  if (!response.ok) {
    throw new Error("Falha ao carregar malha do IBGE.");
  }
...
function resolveLayerByZoom(
  ...
  if (zoom >= LAYER_ZOOM_BREAKPOINTS.escola) {
    return "escola";
  }

  if (zoom >= LAYER_ZOOM_BREAKPOINTS.bairro) {
    return "bairro";
  }
...
export function useMapLayers({
  ...
  const cacheKey = useMemo(() => {
    return recorteId ? `${resolvedLayer}:${recorteId}` : null;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const fallbackCollection = buildMockCollection(
          resolvedLayer,
          currentBackendRecorteId,
        );
